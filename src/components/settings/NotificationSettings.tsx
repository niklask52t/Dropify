...
interface NotificationSettingsProps {
  initialSettings: { emailEnabled: boolean; pushEnabled: boolean } | null;
}
...
export function NotificationSettings({ initialSettings }: NotificationSettingsProps) {
  const [settings, setSettings] = useState({
    ...
    pushEnabled:  initialSettings?.pushEnabled  ?? false,
  });
  ...
  async function saveSettings(updates: Partial<typeof settings>) {
    ...
    await fetch('/api/notifications/settings', {
      ...
      body: JSON.stringify({ email_enabled: next.emailEnabled, push_enabled: next.pushEnabled }),
    });
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        {saved   && <span className="ml-auto flex items-center gap-1 text-xs text-brand"><Check size={12} /> Saved</span>}
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-0.5">Get an email when tracked artists release new music.</p>
            </div>
          </div>
          <button onClick={() => saveSettings({ emailEnabled: !settings.emailEnabled })}
            className={cn('relative rounded-full transition-colors shrink-0', settings.emailEnabled ? 'bg-brand' : 'bg-zinc-700')}
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-0.5">Instant browser notifications for new releases.</p>
