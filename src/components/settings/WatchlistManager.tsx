...
export function WatchlistManager({ watchlist: initial }: WatchlistManagerProps) {
  ...
  async function handleRemove(spotifyId: string) {
    ...
    setWatchlist((w) => w.filter((e) => e.artist.spotify_id !== spotifyId));
    ...
    <div className="card p-5">
      ...
      {watchlist.length === 0 ? (
        <EmptyState
          ...
          action={
            <Link href="/artists" className="btn-secondary text-xs">
              Browse Artists
            </Link>
          }
          className="py-8"
          ...
          {watchlist.map(({ trackId, artist }) => (
            <div key={trackId} className="flex items-center gap-3 p-2 hover:bg-zinc-800/40 rounded-lg group transition-colors">
              <div className="relative w-9 h-9 rounded-lg overflow-hidden bg-zinc-800 shrink-0">
                {artist.image_url ? (
                  <Image src={artist.image_url} alt={artist.name} fill className="object-cover" sizes="36px" />
                ) : (
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  disabled={removing.has(artist.spotify_id)}
