'use client';

import { X } from 'lucide-react';
import type { Artist, ReleaseFilters } from '@/types';

interface ReleaseFiltersProps {
  filters: ReleaseFilters;
  onChange: (filters: ReleaseFilters) => void;
  trackedArtists: Artist[];
}

const RELEASE_TYPES = [
  { value: '', label: 'All types' },
  { value: 'album', label: 'Albums' },
  { value: 'single', label: 'Singles' },
  { value: 'compilation', label: 'Compilations' },
  { value: 'appears_on', label: 'Appears On' },
];

const DATE_RANGES = [
  { value: '', label: 'All time' },
  { value: '7d',  label: 'Last 7 days' },
  { value: '30d', label: 'Last 30 days' },
  { value: '90d', label: 'Last 90 days' },
  { value: '1y',  label: 'Last year' },
];

function getDateFrom(range: string): string {
  if (!range) return '';
  const now = new Date();
  const days: Record<string, number> = { '7d': 7, '30d': 30, '90d': 90, '1y': 365 };
  now.setDate(now.getDate() - (days[range] ?? 0));
  return now.toISOString().split('T')[0];
}

function guessRange(dateFrom?: string): string {
  if (!dateFrom) return '';
  const diff = Math.ceil((Date.now() - new Date(dateFrom).getTime()) / 86_400_000);
  if (diff <= 7) return '7d';
  if (diff <= 30) return '30d';
  if (diff <= 90) return '90d';
  return '1y';
}

export function ReleaseFilters({ filters, onChange, trackedArtists }: ReleaseFiltersProps) {
  const hasActive = filters.artistId || filters.type || filters.dateFrom || filters.search;

  return (
    <div className="flex flex-wrap gap-2">
      <input type="text" value={filters.search ?? ''} placeholder="Search releases…"
        onChange={(e) => onChange({ ...filters, search: e.target.value || undefined })}
        className="input max-w-[200px]" />

      <select value={filters.artistId ?? ''} className="input max-w-[180px] cursor-pointer"
        onChange={(e) => onChange({ ...filters, artistId: e.target.value || undefined })}>
        <option value="">All artists</option>
        {trackedArtists.map((a) => (
          <option key={a.spotifyId} value={a.spotifyId}>{a.name}</option>
        ))}
      </select>

      <select value={filters.type ?? ''} className="input max-w-[150px] cursor-pointer"
        onChange={(e) => onChange({ ...filters, type: e.target.value || undefined })}>
        {RELEASE_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
      </select>

      <select value={guessRange(filters.dateFrom)} className="input max-w-[140px] cursor-pointer"
        onChange={(e) => onChange({ ...filters, dateFrom: getDateFrom(e.target.value) || undefined })}>
        {DATE_RANGES.map((d) => <option key={d.value} value={d.value}>{d.label}</option>)}
      </select>

      {hasActive && (
        <button onClick={() => onChange({})}
          className="flex items-center gap-1.5 px-3 py-2 text-xs text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg border border-zinc-700 transition-colors">
          <X size={12} /> Clear
        </button>
      )}
    </div>
  );
}
